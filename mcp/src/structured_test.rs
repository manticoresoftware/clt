use crate::mcp_protocol::{TestStep, TestStructure};
    ...
    while i < lines.len() {
        ...
        if line.starts_with("––– ") && line.ends_with(" –––") {
            ...
            let step = match statement {
                ...
                Statement::Block => {
                    let block_path =
                        arg.ok_or_else(|| anyhow!("Block statement missing path argument"))?;
...
fn find_and_replace_structure(
    ...
    let start_idx =
        found_at.ok_or_else(|| anyhow!("Old test structure not found in the current file"))?;
...
"#;
    ...
    fn update_test_test_structure() {
        ...
        assert_eq!(
            ...
            Some("This was replaced".to_string())
        );
    ...
    fn update_test_test_structure_not_found() {
        ...
        // Should return error
        let result = replace_test_structure(
            test_file_path.to_str().unwrap(),
            &old_structure,
            &new_structure,
        );
    ...
    fn append_test_test_structure() {
        ...
        // Perform append
        let steps_added =
            append_test_structure(test_file_path.to_str().unwrap(), &append_structure).unwrap();
        ...
        assert_eq!(
            result.steps[2].content,
            Some("This was appended".to_string())
        );
    }
}
