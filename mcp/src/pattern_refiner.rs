use crate::mcp_protocol::{RefineOutputOutput, PatternApplication};
use anyhow::Result;
...
impl PatternRefiner {
    ...
    pub fn refine_output(&self, expected: &str, actual: &str) -> Result<RefineOutputOutput> {
                ...
                ChangeTag::Delete => {
                    ...
                    if !old_text.is_empty() {
                        if let Some(pattern_suggestion) = self.suggest_pattern(old_text) {
                            ...
                            refined_output = refined_output.replace(old_text, &pattern_suggestion.replacement);
                            ...
                            suggestions.push(format!("Replace '{}' with '{}'", 
                ...
                ChangeTag::Insert => {
                    ...
                    if !new_text.is_empty() {
                        if let Some(pattern_suggestion) = self.suggest_pattern(new_text) {
                            suggestions.push(format!("Consider using pattern '{}' for dynamic content: '{}'", 
        ...
        if patterns_applied.is_empty() && suggestions.is_empty() {
            self.find_numeric_differences(expected, actual, &mut refined_output, &mut patterns_applied, &mut suggestions);
        }
    ...
    fn find_numeric_differences(&self, expected: &str, actual: &str, 
        ...
        for (i, (exp_token, act_token)) in expected_tokens.iter().zip(actual_tokens.iter()).enumerate() {
            if exp_token != act_token {
                // Check if both are numbers
                if exp_token.chars().all(|c| c.is_ascii_digit()) && act_token.chars().all(|c| c.is_ascii_digit()) {
                    if let Some(pattern_suggestion) = self.suggest_pattern(exp_token) {
                        *refined_output = refined_output.replace(exp_token, &pattern_suggestion.replacement);
                        patterns_applied.push(PatternApplication {
    ...
    fn suggest_pattern(&self, text: &str) -> Option<PatternSuggestion> {
        ...
        if trimmed_text.chars().all(|c| c.is_ascii_digit()) && trimmed_text.len() > 1 {
            ...
            if trimmed_text.len() == 4 && (trimmed_text.starts_with("19") || trimmed_text.starts_with("20")) {
        ...
        if trimmed_text.contains(':') && (trimmed_text.contains("20") || trimmed_text.contains("19")) {
        ...
        if self.is_uuid(trimmed_text) {
            return Some(PatternSuggestion {
                replacement: "#!/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/!#".to_string(),
                pattern_type: "regex_pattern".to_string(),
            });
        }
    ...
    fn apply_heuristic_patterns(&self, refined_output: &mut String, 
                               patterns_applied: &mut Vec<PatternApplication>,
                               suggestions: &mut Vec<String>) {
    ...
    fn apply_common_variable_patterns(&self, refined_output: &mut String,
                                    patterns_applied: &mut Vec<PatternApplication>,
                                    suggestions: &mut Vec<String>) {
        // Apply patterns for common variables that change between runs
        let common_patterns = vec![
            (r"\b\d{4}-\d{2}-\d{2}\b", "#!/[0-9]{4}-[0-9]{2}-[0-9]{2}/!#", "date"),
            (r"\b\d{2}:\d{2}:\d{2}\b", "#!/[0-9]{2}:[0-9]{2}:[0-9]{2}/!#", "time"),
        ...
        for (regex_pattern, replacement, description) in common_patterns {
            if let Ok(regex) = regex::Regex::new(regex_pattern) {
                if regex.is_match(refined_output) {
                    ...
                    for match_obj in matches.iter().rev() { // Reverse to maintain positions
                        ...
                        suggestions.push(format!("Applied {} pattern for '{}'", description, matched_text));
                    }
    ...
    fn detect_timestamp_pattern(&self, text: &str) -> Option<String> {
        ...
        let timestamp_patterns = vec![
            (r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}", "#!/[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}/!#"),
    ...
    fn find_file_paths(&self, text: &str) -> Option<Vec<String>> {
        ...
        if let Ok(regex) = regex::Regex::new(path_pattern) {
            let matches: Vec<String> = regex.find_iter(text)
    ...
    fn load_patterns() -> Result<HashMap<String, String>> {
        ...
        if patterns.is_empty() {
            ...
            patterns.insert("IPADDR".to_string(), r"[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+".to_string());
            patterns.insert("COMMITDATE".to_string(), r"[a-z0-9]{7}@[0-9]{6}".to_string());
        }
...
mod tests {
    ...
    fn test_refine_output_with_semver() {
        let refiner = PatternRefiner::new().unwrap();
        let result = refiner.refine_output("Version: 1.2.3", "Version: 2.4.6").unwrap();
        
        assert!(result.refined_output.contains("SEMVER") || result.refined_output.contains("1.2.3"));
