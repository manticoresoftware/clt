...
const INIT_CMD: &[u8] = b"export PS1='clt> ';export LANG='en_US.UTF-8' PATH='/bin:/usr/bin:/usr/local/bin:/sbin:/usr/local/sbin' COLUMNS=10000;alias curl='f() { output=$(command curl -s \"$@\"); printf \"%s\" \"$output\"; [ -n \"$output\" ] && [ \"${output: -1}\" != $'\\n' ] && echo; }; f';enable -n exit enable;exec 2>&1;";
